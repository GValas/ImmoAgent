"""
scrapers/superimmo.py — Superimmo (agrégateur d'annonces SSR)
Méthode : httpx + BeautifulSoup — SSR
URL : https://www.superimmo.com/immobilier-vente-maison-{dept-slug}/
...
def _parse_cards(html: str, dept: str) -> list[dict]:
    ...
    cards = (
        ...
        or [a.parent for a in soup.select("a[href*='/immobilier/']") if a.parent]
    )
    ...
    for card in cards:
        try:
            ...
            # Filtre géographique post-extraction
...
async def _scrape_dept(client: httpx.AsyncClient, dept: str,
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        try:
            ...
            print(f"[Superimmo] dept={dept} page={page} → {added} biens")
            if added == 0:
                break
        except Exception as e:
            print(f"[Superimmo] ERR dept={dept}: {e}")
            break
