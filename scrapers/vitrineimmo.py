"""
scrapers/vitrineimmo.py — Vitrine Immobilière (réseau coopératif d'agences)
Méthode : httpx + BeautifulSoup — SSR
URL : https://www.vitrineimmo.fr/vente-maison/{dept-slug}/
...
def _parse_cards(html: str, dept: str) -> list[dict]:
    ...
    cards = (
        soup.select("article.bien")
        or soup.select("div[class*='bien']")
        or soup.select("div[class*='property-card']")
        or soup.select("div[class*='annonce']")
        or soup.select("li.property")
        or [a.parent for a in soup.select("a[href*='/annonce/'], a[href*='/bien/'], a[href*='/vente/maison/']") if a.parent]
    )
...
async def _scrape_dept(client: httpx.AsyncClient, dept: str,
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        try:
            ...
            print(f"[VitrineImmo] dept={dept} page={page} → {added} biens")
            if added == 0:
                break
        except Exception as e:
            print(f"[VitrineImmo] ERR dept={dept}: {e}")
            break
...
async def search(criteres: dict) -> list[dict]:
    ...
    print(f"[VitrineImmo] total: {len(results)} biens")
    return results
