"""scrapers/gsm_immobilier.py — GSM Immobilier (réseau d'agences de Touraine 37 :
Montbazon, Sainte-Maure-de-Touraine, Monts… — stock quasi exclusivement Indre-et-Loire)
...
<script> inline (b64_to_utf8("...")) — même famille qu'immo_mais_pas_que :
  - prodResults["search"] = réfs des 10 biens de la page (ex "VM14070")
  - prodId[ref]           = fiche produit COMPLÈTE (cp, city, pricePrimary,
                            surface, land, rooms, rooms2, details.fr, photos…)
URL pattern : /vente?page={N} (10 biens/page, catalogue unique multi-types ; les
filtres serveur sont ignorés) → post-filtre STRICT code_postal[:2] côté Python,
0 fuite.

Type : prod_type == "house" uniquement (appt/build/land/parking/pro exclus).
Le scraper ne requête que si un département de son stock (37) est demandé.
...
BASE_URL = "https://www.gsm-immobilier.com"
SOURCE = "gsm_immobilier"
LABEL = "GSMImmobilier"
AGENCE = "GSM Immobilier"
# Départements où le réseau a du stock (agences toutes en Indre-et-Loire).
DEPTS_STOCK = {"37"}
MAX_PAGES = 40
PHOTOS_PER_BIEN = 10
...
def _parse_product(ref: str, p: dict) -> dict | None:
    if p.get("prod_type") != "house":        # appt / build / land / parking / pro…
        return None
