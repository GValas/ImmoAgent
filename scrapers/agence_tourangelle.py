"""scrapers/agence_tourangelle.py — Agence Tourangelle (Bléré / Est de Tours, 37 ;
quelques mandats épars hors zone : 77, 86…)

Méthode : scrape_simple (httpx) — SSR HTML, CMS La Boîte Immo « card_bien » v2
(même famille que scrapers/anjou_immobilier.py : listing catalogue unique paginé).
...
Cartes : div.card_bien
  ...
  - Titre    : texte du lien ("Maison 8 pièce(s) 5 chambre(s) 217 m²")
  - CP/Ville : .card_bien__localisation "Bléré (37150)"
  - Prix     : .card_bien__prix "357 000 €"
  - Chambres/surface : li .card_bien__title_part_3 ("5 chambre(s)", "217 m²")
Le terrain/DPE/description ne sont pas sur la carte → laissés à gallery.py.

Leçon vague 1 appliquée : pagination sur les CARTES BRUTES (arrêt seulement quand
une page ne montre plus aucune carte nouvelle), bornes prix/surface appliquées
ensuite — le stock est petit et des pages entières sous prix_min ne doivent pas
tronquer le balayage.

Types conservés : segment d'URL maison/propriété/longère… (appartement, terrain
à bâtir, immeuble… exclus). Gating : ne requête que si le 37 est demandé.
...
BASE_URL = "https://www.agencetourangelle.fr"
SOURCE = "agence_tourangelle"
LABEL = "AgenceTourangelle"
AGENCE = "Agence Tourangelle"
DEPTS_STOCK = {"37"}
MAX_PAGES = 15
PHOTOS_PER_CARD = 10
...
def _parse_card(card) -> dict | None:
    ...
    # id_annonce : id numérique du dernier segment "383-maison-8-pieces-..."
    id_annonce = url
    if parts:
    ...
    if surface is None:  # repli : "217 m²" dans le titre du lien
