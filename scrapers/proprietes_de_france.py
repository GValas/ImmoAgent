"""
scrapers/proprietes_de_france.py — Propriétés de France (demeures rurales prestige)
Méthode : httpx + BeautifulSoup — SSR
URL : https://www.proprietes-de-france.com/achat/maison/{region-slug}/
...
BASE = "https://www.proprietes-de-france.com"
...
def _parse_cards(html: str, dept: str) -> list[dict]:
    ...
    cards = (
        ...
        or [a.parent for a in soup.select("a[href*='/annonce/'], a[href*='/bien/']") if a.parent]
    )
    ...
    for card in cards:
        try:
            ...
            if m_ha:
                try:
                    terrain = float(m_ha.group(1).replace(",", ".")) * 10_000
                except Exception:
                    pass
...
async def _scrape_dept(client: httpx.AsyncClient, dept: str,
    ...
    regions = _REGION_MAP.get(dept, [])
    biens: list[dict] = []
    seen_ids: set[str] = set()

    urls_to_try = []
    for r in regions:
        urls_to_try.append(f"{BASE}/achat/maison/{r}/")
        urls_to_try.append(f"{BASE}/vente/maison/{r}/")
    urls_to_try.append(f"{BASE}/recherche/?type=maison&departement={dept}&transaction=achat")
    ...
    for base_url in urls_to_try:
        for page in range(1, MAX_PAGES + 1):
            ...
            try:
                ...
                print(f"[ProprietesDeFrance] dept={dept} page={page} → {added} biens")
                if added == 0:
                    break
            except Exception as e:
                print(f"[ProprietesDeFrance] ERR dept={dept}: {e}")
                break
...
async def search(criteres: dict) -> list[dict]:
    ...
    print(f"[ProprietesDeFrance] total: {len(results)} biens")
    return results
