"""scrapers/immo_mais_pas_que.py — Immo mais pas que (agence Montval-sur-Loir /
Saint-Calais, Sarthe 72 — stock aussi en 41/37, antenne Savoie 73 hors zone)

Méthode : scrape_simple (httpx) — CMS **Netty** (React CSR) : le HTML ne contient
pas de cartes, MAIS l'état complet de la page est embarqué en base64 dans un
<script> inline (window._TEMPLATE_DATA via b64_to_utf8("...")) :
  - prodResults["search"] = réfs des 12 biens de la page (ex "VM5204")
  - prodId[ref]           = fiche produit COMPLÈTE (cp, city, pricePrimary,
                            surface, land, rooms, rooms2, details.fr, photos…)
URL pattern : /vente?page={N} (12 biens/page, catalogue unique multi-dept ; les
filtres serveur prix/surface/budget sont IGNORÉS) → post-filtre STRICT
code_postal[:2] côté Python, 0 fuite (écarte l'antenne Savoie 73 / Var 83).

Type : prod_type == "house" uniquement (appt/land/parking/pro exclus).
Le scraper ne requête que si un département de son stock (72/41/37) est demandé.
...
BASE_URL = "https://www.immomaispasque.com"
SOURCE = "immo_mais_pas_que"
LABEL = "ImmoMaisPasQue"
AGENCE = "Immo mais pas que"
# Départements où l'agence a du stock (le reste : Savoie/Var, hors zone).
DEPTS_STOCK = {"72", "41", "37"}
MAX_PAGES = 25
PHOTOS_PER_BIEN = 10
