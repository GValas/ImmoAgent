"""scrapers/aisling_immobilier.py — Aisling Immobilier (agence de Joué-lès-Tours,
agglo de Tours 37 — stock exclusivement Indre-et-Loire)
...
gsm_immobilier :
  - prodResults["search"] = réfs des 12 biens de la page (ex "VM380")
  - prodId[ref]           = fiche produit COMPLÈTE (cp, city, pricePrimary,
                            surface, land, rooms, rooms2, details.fr, photos…)
URL pattern : /vente?page={N} (12 biens/page, catalogue unique multi-types ; les
filtres serveur sont ignorés) → post-filtre STRICT code_postal[:2] côté Python,
0 fuite.

Type : prod_type == "house" uniquement (appt/pro/comm/land/parking exclus).
...
BASE_URL = "https://www.aisling-immobilier.fr"
SOURCE = "aisling_immobilier"
LABEL = "AislingImmobilier"
AGENCE = "Aisling Immobilier"
# Département où l'agence a son stock (agglo de Tours).
DEPTS_STOCK = {"37"}
MAX_PAGES = 30
PHOTOS_PER_BIEN = 10
...
def _parse_product(ref: str, p: dict) -> dict | None:
    if p.get("prod_type") != "house":        # appt / pro / comm / land / parking…
        return None
